...
1. Install uv and sync the environment: `uv sync --extra dev`.
2. Set `SENDGRID_API_KEY`, `SENDGRID_FROM_EMAIL`, and `SENDGRID_TO_EMAIL`.
3. Run `uv run paper-emailer --dry-run` to preview the email.
4. Run `uv run paper-emailer --show-email` if you want to print the rendered MIME message.
...
6. The workflow installs uv, syncs the environment from `pyproject.toml`, and runs the digest with `uv run`.
